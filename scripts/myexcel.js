...
(function () {
    ...
    function setCell(cell, value, style, isstring, colspan) {
        if (value != undefined) cell.v = value;// else cell.v="";
    ...
    $JExcel.new = function (defaultFont) {
        ...
        excel.merge = function (s,m) {
			s=sheets.get(s);
			s.mergeCells.push(m);
		}
        excel.mergeRow = function (s,row,from,to) {
			var m={from:cellName(from,row+1),to:cellName(to,row+1)};
			s=sheets.get(s);
			s.mergeCells.push(m);
		}
        excel.mergeCol = function (s,from,to,col) {
			var m={from:cellName(col,from+1),to:cellName(col,to+1)};
			s=sheets.get(s);
			s.mergeCells.push(m);
		}
        ...
        excel.generate = function (filename) {
			...
			var res=zip.generate({ type: "blob",mimeType:"application/vnd.ms-excel" });
			saveAs(res,filename);
	}
        return excel;
    }
